...
Each `FAVS_PhysicsForce` carries a mode, a location in cm, a force in centinewtons, an optional brake torque in Nm, the delta time, an acceleration-change flag, and the originating wheel index.

Logging that list for a vehicle you already understand shows exactly what AVS applies, in order. It will usually tell you whether the drivetrain override is needed at all.
...
Set the explicit override flags on individual outputs to replace that wheel's stock drive torque or persistent angular velocity. Wheels you do not flag keep stock behavior, so you can override one axle and leave the rest of the vehicle unchanged.
...
A worked example: a simple open differential that sends torque to whichever driven wheel has more grip.
...
Wheels left unflagged keep stock behavior, so an override can cover one axle only.
...
Filtering is the safest use: dropping or scaling specific forces under conditions you control. Adding your own forces is also straightforward. Replacing the whole set means reimplementing suspension and traction.
