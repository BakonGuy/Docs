...
`BrakeSqueal` ships as an example. It does not use the standard output block, and implements its own audio-only output instead.
...
Use **Requirements Met** instead of checking inside Tick. Return false when the effect does not apply to this wheel, such as no sound assigned or a driving-wheel-only effect on an undriven wheel, and the effect is never created.
...
Two convenience functions on the effect base class:

- `GetContactStartedThisFrame` — true during the frame the wheel regained contact. This is how you fire a one-shot on landing without tracking state yourself.
...
Prefer this over a custom output. Anyone configuring your effect then gets the same placement, intensity range, audio, and particle settings used by the built-in effects. Write your own output only when you need something the standard block cannot express, as Brake Squeal does with its three audio parameters.



## Runtime Instances and Effect State
...
**Surface effects are recreated** when contact or surface changes, so their state resets with them. If your effect needs to remember something across a surface change, it has to be a global effect.
